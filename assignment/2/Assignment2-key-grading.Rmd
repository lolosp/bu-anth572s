...
## Problem 1. Final grades in bioanth *(10pts total)*
...
#### Q1.1. Do you have one or two samples? What is your sample size? What kind of test should you use? *(2 pts, 0.5 for the first two and 1 for the last Q)*

One sample. Sample size is 20. You should use a one tailed t-test.
...
H~0~: The mean score is 67.5
H~1~: The mean score is greater than 67.5
...
ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  ...
  geom_area(stat = 'function', fun = dnorm, args = list(mean = 0, sd = 1), fill = "red", alpha = 0.4, xlim = c(crit, 4))+ # modify the xlim argument 
...
## Problem 2. Guns laws and violent crime rates in the US *(14pts)*
...
Two samples. 21 states without a law, 29 states with a law. Two sample t-test 
...
H~0~: There is no difference in the mean violent crime rate between states with and without a shall issue law
H~1~: There is a difference in the mean violent crime rate between states with and without a shall issue law

#### Q2.4. Is this a one or two tailed test?
*(1pt)*

Two tailed -- would accept one tailed, but then the test should be run accordingly
...
2. Normality of your variables
3. Equality of variance

The independence of observation is met because each observation is for a different state.
...
```{r}
guns.no <- filter(guns, law == "no")
guns.ys <- filter(guns, law == "yes")

shapiro.test(guns.no$violent)
shapiro.test(guns.ys$violent)
```
...
```{r}
leveneTest(violent ~ law, data = guns)
```
#### Q2.8. Given the results of your assumptions testing, which version of the test should you use and why?
*(1pt)*

Student t-test because the samples have equal variance -- Will accept Welch's test with the argument that it doesnt reduce power much.
...
If one tailed, needs the alternative = "greater" argument
```{r}
t.test(violent ~ law, data = guns, var.equal = TRUE)
t.test(violent ~ law, data = guns, var.equal = TRUE, alternative = "greater")
