...
```{r}
ggplot(wine, aes(x = country, y = price))+
  geom_boxplot()
...
```{r}
shapiro.test(wine$price[wine$country == "France"])
shapiro.test(wine$price[wine$country == "Italy"])
shapiro.test(wine$price[wine$country == "US"])
```
...
```{r}
kruskal.test(price ~ country, data = wine) 
...
## Problem 2: Height in the MLB *(8pts)*
...
## Problem 3: Sodium intake across three groups *(11pts)*
